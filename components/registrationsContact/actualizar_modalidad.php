...
if (isset($_POST['id']) && isset($_POST['nuevaModalidad'])) {
    // Depuración: Verifica los valores recibidos
    error_log("ID recibido: " . $_POST['id']);
    error_log("Nueva Modalidad recibida: " . $_POST['nuevaModalidad']);

    $id = $_POST['id'];
    $nuevaModalidad = $_POST['nuevaModalidad'];
    ...
    // Consulta SQL para actualizar la modalidad
    $updateSql = "UPDATE user_register SET mode = ? WHERE number_id = ?";
    $stmt = $conn->prepare($updateSql);

    if ($stmt) {
        // Vincular los parámetros para la consulta preparada
        $stmt->bind_param('si', $nuevaModalidad, $id);
