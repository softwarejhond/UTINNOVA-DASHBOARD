...
$sql = "SELECT user_register.*, departamentos.departamento,
               g.id_bootcamp, g.bootcamp_name,
               g.id_leveling_english, g.leveling_english_name,
               g.id_english_code, g.english_code_name,
               g.id_skills, g.skills_name
        FROM user_register
    ...
    <div class="table-responsive">
                        ...
                        <table id="listaInscritos" class="table table-hover table-bordered">
                            <thead class="thead-dark text-center">
                                <tr>
                                    ...
                                    <th>Horario</th>
                                    <th>Tecnico</th>
                                    <th>Ingles nivelador</th>
                                    <th>English Code</th>
                                    <th>Habilidades de poder</th>
                                </tr>
                            </thead>
                            <tbody class="text-center">
                                ...
                                ?>
                                    <tr data-type-id="<?php echo htmlspecialchars($row['typeID']); ?>"
                                        ...
                                        <td>
                                            <?= ($row['id_bootcamp'] && $row['bootcamp_name']) ? htmlspecialchars($row['id_bootcamp'] . ' - ' . $row['bootcamp_name']) : '' ?>
                                        </td>
                                        <td>
                                            <?= ($row['id_leveling_english'] && $row['leveling_english_name']) ? htmlspecialchars($row['id_leveling_english'] . ' - ' . $row['leveling_english_name']) : '' ?>
                                        </td>
                                        <td>
                                            <?= ($row['id_english_code'] && $row['english_code_name']) ? htmlspecialchars($row['id_english_code'] . ' - ' . $row['english_code_name']) : '' ?>
                                        </td>
                                        <td>
                                            <?= ($row['id_skills'] && $row['skills_name']) ? htmlspecialchars($row['id_skills'] . ' - ' . $row['skills_name']) : '' ?>
                                        </td>
                ...
                <div class="offcanvas offcanvas-end" tabindex="-1" id="selectedUsersList" aria-labelledby="selectedUsersListLabel">
                    <div class="offcanvas-header">
                        <h5 class="offcanvas-title" id="selectedUsersListLabel">
                            <i class="bi bi-person-check"></i> seleccionados (<span id="selectedCount">0</span>)
                        </h5>
                    ...
                    <div class="offcanvas-body">
                        <div class="m-3">
                            <button id="enrollSelectedUsers" class="btn bg-magenta-dark text-white w-100">
                                <i class="bi bi-patch-check-fill"></i> Gestionar Seleccionados
                            </button>
                        </div>
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                Swal.fire({
                    title: 'Atención',
                    html: 'Este módulo es para <b>cambiar a personas de componente técnico</b>.<br><br>Las asistencias de los demás componentes (inglés nivelador, English Code y habilidades de poder) <b>se mantendrán</b>.',
                    icon: 'info',
            ...
            function confirmBulkEnrollment(usersToEnroll) {
                if (usersToEnroll.length === 0) {
                    Swal.fire('Error', 'Por favor seleccione al menos un campista', 'error');
                    return;
                }

                Swal.fire({
                    title: 'Confirmar matrícula',
                    text: `¿Está seguro que desea cambiar a ${usersToEnroll.length} campistas seleccionados?`,
                    icon: 'warning',
            ...
            async function processEnrollments(usersToEnroll) {
                ...
                for (const formData of usersToEnroll) {
                    try {
                        // 1. Desmatricular primero
                        const deleteResponse = await fetch('components/activeMoodle/deleteMatricula.php', {
                            method: 'POST',
                            headers: {
                        ...
                        if (!deleteData.success) {
                            throw new Error(`Error al desmatricular: ${deleteData.message}`);
                        }
                        ...
                        if (!enrollResponse.ok || !enrollData.success) {
                            throw new Error(`Error en la matrícula: ${enrollData?.message || 'Error desconocido'}`);
                        }

                        // 3. Actualizar attendance_records
                        const updateAttendanceResponse = await fetch('components/registerMoodle/update_attendance_courses.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                            body: JSON.stringify({
                                number_id: formData.number_id,
                                id_leveling_english: formData.id_leveling_english,
                                id_english_code: formData.id_english_code,
                                id_skills: formData.id_skills
                            })
                        });
                        const updateAttendanceData = await updateAttendanceResponse.json();
                        if (!updateAttendanceData.success) {
                            errors.push({
                                student: formData.number_id,
                                message: `Matrícula exitosa, pero error al actualizar asistencia: ${updateAttendanceData.message}`,
        ...
        `;
            ...
            async function sendEnrollmentEmail(userData, carnetFilePath = null) { // Añadir carnetFilePath como parámetro opcional
                try {
                    const payload = {
                    ...
                    if (carnetFilePath) {
                        payload.carnet_file_path = carnetFilePath; // Añadir la ruta del carnet al payload si está disponible
                    }
