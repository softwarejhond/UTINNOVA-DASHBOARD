...
function exportDataToExcel($conn)
{
    ...
    AND user_register.number_id NOT IN (
        ...
        WHERE user_register.directed_base = '1'
    )
...
try {
    $reportDir = __DIR__ . '/../../reports/semanal_L2/';
    
    // Eliminar archivos anteriores que coincidan con el patrón
    $pattern = $reportDir . 'informe_semanal_L2_*.xlsx';
    ...
    $fecha = date('dmy_His');
    $filename = $reportDir . 'informe_semanal_L2_' . $fecha . '.xlsx';
