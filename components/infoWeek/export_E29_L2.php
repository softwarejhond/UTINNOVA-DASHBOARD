...
function exportDataToExcel($conn)
{
    ...
    // Eliminar consulta y mapeo de la API DIVIPOLA
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // Construir fila de datos SIN mapeo, usando los valores directos
            $data[] = [
                ...
                'Nombre1' => strtoupper($row['first_name']),
                'Nombre2' => strtoupper($row['second_name']),
                'Apellido1' => strtoupper($row['first_last']),
                'Apellido2' => strtoupper($row['second_last']),
                'Fecha_nacimiento' => fechaAExcel($row['birthdate']),
                'Correo' => $row['email'],
                'Codigo_epartamento' => $row['department'],
                'Departamento' => strtoupper($row['departamento']),
                'Región' => 'Región 8 Lote 2',
                ...
                'En formación' => $tieneProfesor === 'En formación' ? 'SI' : 'NO',
    ...
    header('Content-Disposition: attachment;filename="informe_E29_L2_' . date('Y-m-d') . '.xlsx"');
