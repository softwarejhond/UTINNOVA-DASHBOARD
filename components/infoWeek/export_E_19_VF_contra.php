...
function exportDataToExcel($conn)
{
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $data[] = [
                ...
                'creationdate' => date('d/m/Y', strtotime($row['creationDate'])),
                ...
                'Fecha_inscripción' => date('d/m/Y', strtotime($row['creationDate'])),
                'Curso_bootcamp_donde_se_matriculo' => $row['bootcamp_name'],
                'Origen' => 'UTI-R8L1',
                'Matriculado' => $estaEnGroups ? 'SI' : 'NO',
                'Fecha_matricula' => $row['creation_date'] ? date('d/m/Y', strtotime($row['creation_date'])) : '',
                ...
                'Fecha de inicio de la formación' => $row['bootcamp_start_date'] ? date('d/m/Y', strtotime($row['bootcamp_start_date'])) : '',
                'Cohorte_asignación (1,2,3,4,5,6,7 o 8)' => $row['bootcamp_cohort'],
                'Año Cohorte asignación' => $row['bootcamp_start_date'] ? date('Y', strtotime($row['bootcamp_start_date'])) : '',
                'Cohorte de formación (1,2,3,4,5,6,7 o 8)' => $row['bootcamp_cohort'],
                'Año_cohorte_formación' => $row['bootcamp_start_date'] ? date('Y', strtotime($row['bootcamp_start_date'])) : '',
                'Fecha_de_terminacion' => $row['bootcamp_end_date'] ? date('d/m/Y', strtotime($row['bootcamp_end_date'])) : '',
                'Cumple requisitos' => in_array($row['statusAdmin'], ['1', '3', '6', '8', '9', '10']) ? 'SI' : '',
                'Tipo de formación_matricula' => $row['group_mode'],
                'Observaciones (menos de 50 cracteres)' => '',

                'Codigo del curso' => $row['id_bootcamp'],
                'Nombre del curso' => $row['bootcamp_name'],
                '% de asistencia' => $estaEnGroups ? (isset($attendanceCount[$row['number_id']]) ? round(($attendanceCount[$row['number_id']] / 159) * 100, 2) . '%' : '0%') : '',
    ...
    // Columnas A a AK (1-37): color amarillo oscuro
    $sheet->getStyle('A1:AK1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FFB8860B'); // Amarillo oscuro (Dark Golden Rod)

    // Columnas AL a AY (38-51): color verde oscuro
    $sheet->getStyle('AL1:AY1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FF006400'); // Verde oscuro (Dark Green)

    // Columnas AZ a BN (52-66): color amarillo brillante
    $sheet->getStyle('AZ1:BN1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FFFFFF00'); // Amarillo brillante (Yellow)

    // Aplicar fuente en negrita a todos los encabezados
    $sheet->getStyle('A1:' . $lastColumn . '1')->getFont()->setBold(true);

    // Para mejor legibilidad en el verde oscuro, cambiar el texto a blanco
    $sheet->getStyle('AL1:AY1')->getFont()->getColor()->setARGB('FFFFFFFF');
