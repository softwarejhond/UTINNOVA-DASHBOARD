...
function exportDataToExcel($conn)
{
    ...
    AND user_register.birthdate IS NOT NULL
    AND user_register.birthdate != ''
    AND user_register.birthdate < '" . CURRENT_YEAR . "-" . date('m-d') . "'
    AND user_register.number_id IS NOT NULL
    AND user_register.number_id != ''
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if (empty($row['number_id']) || empty($row['first_name']) || empty($row['birthdate'])) {
                continue; // Saltar este registro si faltan datos críticos
            }

            // Asegurar que los campos de texto no contengan caracteres problemáticos
            ...
            $row['municipio'] = $row['municipio'] ?? 'SIN MUNICIPIO';
            $row['departamento'] = $row['departamento'] ?? 'SIN DEPARTAMENTO';
            $row['department'] = $row['department'] ?? '0';
            $row['municipality'] = $row['municipality'] ?? '0';
            ...
            // Calcular edad con validación
            try {
                $birthday = new DateTime($row['birthdate']);
                $now = new DateTime();
                $age = $now->diff($birthday)->y;
            } catch (Exception $e) {
                $age = 0; // Valor por defecto si la fecha es inválida
            }
