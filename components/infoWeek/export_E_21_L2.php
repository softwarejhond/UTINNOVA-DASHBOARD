...
function exportDataToExcel($conn)
{
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $data[] = [
                'Operador' => 'UNIÓN TEMPORAL TALENTO TICS',
                'id' => $row['id'],
                ...
                'Origen' => 'UTI-R8L2',
                'Fecha inscripción' => !empty($row['creationDate']) ? date('d/m/Y', strtotime($row['creationDate'])) : '',

                'Cumple requisitos' => in_array($row['statusAdmin'], ['1', '3', '6', '8', '9', '10']) ? 'SI' : '',
                'Cohorte asignación' => $row['bootcamp_cohort'],
                'Año cohorte' => !empty($row['start_date']) ? date('Y', strtotime($row['start_date'])) : '',
                'Fecha asignación' => !empty($row['creation_date']) ? date('d/m/Y', strtotime($row['creation_date'])) : '',

                'Matriculado' => $estaEnGroups ? 'SI' : 'NO',
                'Fecha matricula' => !empty($row['creation_date']) ? date('d/m/Y', strtotime($row['creation_date'])) : '',
                'Curso bootcamp donde se matriculo' => $row['bootcamp_name'],
                ...
                'En formación' => $tieneProfesor,
                'Cohorte' => $row['bootcamp_cohort'],
    ...
    // Columnas A a AA (1-27): color amarillo oscuro
    $sheet->getStyle('A1:AA1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FFB8860B'); // Amarillo oscuro (Dark Golden Rod)

    // Columnas AB a AF (28-32): color naranja oscuro
    $sheet->getStyle('AB1:AF1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FFFF8C00'); // Naranja oscuro (Dark Orange)

    // Columnas AG a AJ (33-36): color aqua marina claro
    $sheet->getStyle('AG1:AJ1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FF7FFFD4'); // Aqua marina claro (Aquamarine)

    // Columnas AK a AR (37-44): color azul celeste
    $sheet->getStyle('AK1:AR1')
        ->getFill()->setFillType(Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FF87CEEB'); // Azul celeste (Sky Blue)
