...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $title = "Mentoría {$course_name}, {$fecha_titulo}";
    $url = "https://dashboard.utinnova.co/asistenciaMentorias.php?fecha={$fecha_url}&curso={$course_id}";
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO qr_mentorias (title, url, image_filename, clases_equivalentes, authorized_by, authorized, created_by) VALUES (?, ?, ?, ?, 0, 0, ?)");
        if (!$stmt) {
            throw new Exception("Error en la preparación de la consulta: " . $conn->error);
        }
        $stmt->bind_param("sssii", $title, $url, $image_filename, $clases_equivalentes, $created_by);
